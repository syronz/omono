...
// EacIncomeStatement for injecting auth eacrepo
type EacIncomeStatementServ struct {
	Repo   eacrepo.BalanceSheetRepo
	Engine *core.Engine
}

// ProvideEacIncomeStatementService for transaction is used in wire
func ProvideEacIncomeStatementService(p eacrepo.BalanceSheetRepo) EacIncomeStatementServ {
	return EacIncomeStatementServ{
		Repo:   p,
		Engine: p.Engine,
	}
}
...
func (p *EacIncomeStatementServ) BalanceSheet(params param.Param, level string) (balanceSheet []eacmodel.BalanceSheet, err error) {
	var depth uint64
...
func (p *EacIncomeStatementServ) FetchBalances(params param.Param, parentAccount types.RowID, depth uint64, accounts []basmodel.Account, balances []eacmodel.Balance) (balanceSheet []eacmodel.BalanceSheet, balance float64, err error) {
...
func fetchinge(mainAccount types.RowID, accounts []basmodel.Account) (nameEn, nameKu, nameAr string) {
