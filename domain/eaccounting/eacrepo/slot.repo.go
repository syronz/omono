...
func (p *SlotRepo) TxSave(db *gorm.DB, slot eacmodel.Slot) (u eacmodel.Slot, err error) {
	if err = db.Table(eacmodel.SlotTable).Save(&slot).Error; err != nil {
		err = p.dbError(err, "E1435079", slot, corterm.Updated)
	}

	db.Table(eacmodel.SlotTable).Where("id = ?", slot.ID).Find(&u)
	return
}
...
func (p *SlotRepo) TxCreate(db *gorm.DB, slot eacmodel.Slot) (u eacmodel.Slot, err error) {
	if err = db.Table(eacmodel.SlotTable).Create(&slot).Scan(&u).Error; err != nil {
...
func (p *SlotRepo) TxDelete(db *gorm.DB, slot eacmodel.Slot) (err error) {
	now := time.Now()
	slot.DeletedAt = &now
	if err = db.Table(eacmodel.SlotTable).Save(&slot).Error; err != nil {
...
func (p *SlotRepo) TxLastSlot(db *gorm.DB, slotIn eacmodel.Slot) (slot eacmodel.Slot, err error) {
	// err = db.Clauses(clause.Locking{Strength: "UPDATE"}).Table(eacmodel.SlotTable).
	err = db.Table(eacmodel.SlotTable).
...
func (p *SlotRepo) TxRegulateBalances(db *gorm.DB, slotIn eacmodel.Slot, adjust float64) (err error) {
	err = db.Table(eacmodel.SlotTable).
		Where("company_id = ? AND account_id = ? AND currency_id = ? AND post_date > ?",
			slotIn.CompanyID, slotIn.AccountID, slotIn.CurrencyID, slotIn.PostDate).
		Update("balance", gorm.Expr("balance + ?", adjust)).Error
...
func (p *SlotRepo) TxRegulateBalancesSave(db *gorm.DB, slotIn eacmodel.Slot, adjust float64) (err error) {
	err = db.Table(eacmodel.SlotTable).
		Where("company_id = ? AND account_id = ? AND currency_id = ? AND post_date >= ? AND id > ?",
			slotIn.CompanyID, slotIn.AccountID, slotIn.CurrencyID, slotIn.PostDate, slotIn.ID).
		Update("balance", gorm.Expr("balance + ?", adjust)).Error
...
func (p *SlotRepo) TxUpdateBalance(db *gorm.DB, b eacmodel.Balance) (err error) {
